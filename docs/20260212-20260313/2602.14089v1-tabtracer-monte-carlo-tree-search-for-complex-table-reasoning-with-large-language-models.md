...
tags: ["query:slm-rl"]
score: 7.0
evidence: 用于多步工具调用和复杂推理的蒙特卡洛树搜索
...
## 摘要
大语言模型（LLMs）已成为自然语言表格推理的强大工具，该领域主要存在两类方法。基于提示（Prompt-based）的方法依赖于纯语言推理或不具备步骤级验证的单次程序生成。基于智能体（Agent-based）的方法在闭环中使用工具，但验证往往是局部的且回溯能力有限，导致错误传播并增加了成本。此外，它们依赖于链式或束式（beam-style）轨迹，这些轨迹通常具有组合冗余，导致高昂的 token 成本。在本文中，我们提出了 TabTracer，这是一个智能体框架，它在中间表格状态上协调多步工具调用，并具有用于验证和回滚的显式状态跟踪。首先，它通过类型化操作以及轻量级数值和格式检查来强制执行步骤级验证，以提供可靠的奖励并抑制幻觉。其次，执行反馈蒙特卡洛树搜索维护候选表格状态的搜索树，并利用反向传播的反思分数来指导 UCB1 选择，并通过版本化快照实现回滚。第三，它通过预算感知剪枝、去重以及带有单调性门的显式状态哈希来减少冗余，从而降低 token 成本。在 TabFact、WikiTQ 和 CRT 数据集上的全面评估表明，TabTracer 在准确率上优于最先进的基准模型高达 6.7%，同时将 token 消耗降低了 59--84%。
