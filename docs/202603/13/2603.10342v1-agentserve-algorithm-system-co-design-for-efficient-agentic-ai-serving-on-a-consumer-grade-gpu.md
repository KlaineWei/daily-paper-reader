...
## 摘要
大语言模型 (LLM) 越来越多地被部署为 AI 智能体，在简短的“推理-行动”循环中运行，将模型计算与外部调用交织在一起。与传统的聊天应用不同，这些智能体工作负载要求推理服务系统在来自不同 AI 智能体的多个请求并发到达时，平衡低延迟、稳定的 Token 发射速率和吞吐量。受隐私、合规性和成本限制的驱动，最近的部署呈现出在消费级 GPU 上本地运行小语言模型 (SLM) 的趋势。当异构请求在单个 GPU 上重叠时，长预填充 (prefill) 和短解码 (decode) 会争夺资源，产生队头阻塞 (head-of-line blocking)，从而破坏交互性能的稳定性。通过分析智能体工作负载，我们观察到它们的执行自然地分为：处理长系统提示词的冷预填充 (cold prefill)、将工具输出附加到缓存上下文的恢复预填充 (resume prefill)，以及对延迟敏感的短解码。与传统的聊天机器人服务相比，这种混合负载加剧了资源竞争。我们提出了 AgentServe，这是一个单 GPU 服务系统，通过将预填充与解码隔离、对恢复预填充应用动态预算，以及通过预先建立的具有自适应控制的 CUDA Green Context 插槽分配 GPU 资源，确保在此类条件下稳定的多智能体执行。评估结果表明，AgentServe 在保持竞争性吞吐量的同时，显著提高了延迟稳定性，在不同设置下，相比最先进的基准模型，其首字延迟 (TTFT) 提升高达 2.8 倍，逐字延迟 (TPOT) 提升高达 2.7 倍。
