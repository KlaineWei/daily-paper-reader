...
title_zh: Ares：面向高效大语言模型智能体的自适应推理力度选择
...
tldr: "针对大语言模型智能体在长链思维推理中面临的高昂推理成本问题，本文提出了 Ares 框架。该框架通过动态选择每一步所需的推理强度（如高/中/低），在保证任务成功率的同时显著降低成本。Ares 利用轻量级路由预测每步的最优推理级别，并通过专门的数据生成管线进行训练。实验表明，Ares 在多个智能体基准测试中可减少高达 52.7% 的推理 Token 使用量，实现了效率与性能的平衡。"
selection_source: fresh_fetch
motivation: 现有的思维模型推理成本高昂，且固定或随机的推理强度选择策略无法在保证准确性的同时有效降低成本。
method: 提出 Ares 框架，通过轻量级路由动态预测每步任务所需的最低推理级别，并利用自动化管线生成训练数据。
result: "在 TAU-Bench 和 WebArena 等多个任务中，Ares 在几乎不损失成功率的前提下，将推理 Token 消耗降低了多达 52.7%。"
conclusion: 动态推理强度选择是提升大模型智能体效率的有效途径，Ares 为实现高性能且低成本的智能体提供了即插即用的解决方案。
---

## 摘要
由具备思考能力的大语言模型（LLM）驱动的现代智能体通过长链条思维（CoT）推理实现了高准确率，但也产生了巨大的推理成本。虽然许多 LLM 现在支持可配置的推理级别（例如：高/中/低），但静态策略通常效果不佳：在每一步都使用低力度模式会导致性能显著下降，而随机选择则无法保持准确率或提供实质性的成本降低。然而，智能体应当将高推理力度保留给困难步骤（如导航复杂的网站结构），而在简单步骤（如打开目标 URL）中使用较低力度的模式。在本文中，我们提出了 Ares，这是一个专为多步智能体任务量身定制的逐步动态推理力度选择框架。Ares 采用轻量级路由，根据交互历史预测每一步最合适的最低推理级别。为了训练该路由，我们开发了一个数据生成流水线，用于识别成功完成步骤所需的最小推理力度。随后，我们对路由进行微调以预测这些级别，从而实现对任何 LLM 智能体的即插即用集成。我们在多种智能体任务上评估了 Ares，包括用于工具使用智能体的 TAU-Bench、用于深度研究智能体的 BrowseComp-Plus 以及用于 Web 智能体的 WebArena。实验结果表明，与固定的高力度推理相比，Ares 将推理 Token 使用量减少了高达 52.7%，同时对任务成功率的影响微乎其微。
