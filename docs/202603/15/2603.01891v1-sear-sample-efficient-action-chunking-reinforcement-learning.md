...
motivation: 现有的动作分块方法主要针对离线设置，在纯在线强化学习中因评论家网络难以评估复杂的动作序列而面临严峻的挑战。
method: 提出 SEAR 算法，利用动作块的时间结构并采用退避视界策略，有效结合了小分块和大分块的各自优势。
result: 在 Metaworld 基准测试中，SEAR 在使用大尺寸动作分块时，其性能和样本效率均显著超越了现有的最先进在线强化学习方法。
conclusion: SEAR 证明了通过优化时间结构利用方式，动作分块技术可以成功应用于并显著增强纯在线强化学习的表现。
---
