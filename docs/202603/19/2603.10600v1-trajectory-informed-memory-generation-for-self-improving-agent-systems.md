...
tags: ["query:slm-rl"]
score: 6.0
evidence: 从智能体执行轨迹中提取可操作的学习经验以实现自我改进
tldr: 针对LLM智能体在重复任务中效率低下且易犯错的问题，本文提出一种基于轨迹感知的记忆生成框架。该框架通过分析执行轨迹，提取策略、恢复和优化三类指导性经验，并利用自适应检索系统在未来任务中注入相关知识。实验表明，该方法在AppWorld基准测试中显著提升了任务成功率，尤其在复杂任务上表现优异，实现了智能体的自我进化。
selection_source: fresh_fetch
motivation: 现有的LLM智能体往往无法从过去的执行经验中学习，导致其在面对相似任务时重复低效模式或无法从错误中恢复。
method: 开发了一个包含轨迹智能提取、决策归因分析、上下文学习生成和自适应记忆检索四个核心组件的自动化经验提取与利用框架。
result: "在AppWorld基准测试中，该框架使场景目标完成率最高提升了14.3个百分点，在复杂任务上的性能提升尤为显著，相对增长达149%。"
conclusion: 通过从执行轨迹中提取结构化且具因果关系的经验，并进行精准的上下文检索，可以有效实现智能体系统的自我改进和性能优化。
---
