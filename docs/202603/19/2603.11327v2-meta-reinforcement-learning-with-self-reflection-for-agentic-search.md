...
tags: ["query:slm-rl"]
score: 7.0
evidence: 用于代理搜索和上下文内探索的元强化学习
tldr: 本研究针对智能体搜索任务中稀疏奖励和单回合优化受限的问题，提出了MR-Search框架。该框架采用上下文元强化学习方法，通过在回合间生成显式的自我反思并将其作为上下文，使智能体能够根据历史经验动态调整搜索策略。此外，引入了多轮强化学习算法以实现细粒度的信用分配。实验表明，MR-Search在八个基准测试中显著提升了搜索效率和泛化能力。
selection_source: fresh_fetch
motivation: 传统的强化学习在处理搜索任务时往往局限于单回合优化且面临奖励稀疏问题，难以在测试时通过历史经验进行自我改进。
method: 提出一种结合自我反思的上下文元强化学习框架，利用跨回合的显式反思指导后续探索，并采用多轮RL算法进行细粒度的信用分配。
result: "在八个基准测试中，MR-Search 相比传统强化学习基准实现了 9.2% 到 19.3% 的相对性能提升，展现出极强的泛化性。"
conclusion: 通过引入自我反思和跨回合元学习机制，MR-Search 有效增强了搜索智能体在测试阶段的上下文探索与策略自适应能力。
---
