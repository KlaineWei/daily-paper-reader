...
motivation: 强化学习智能体在现实部署中常因外部干扰和模型不确定性导致性能大幅下降，亟需提升其鲁棒性。
method: 提出一种基于极小极大优化的MMDDPG框架，通过分式目标函数平衡任务表现与干扰强度以稳定对抗训练。
result: 在MuJoCo连续控制任务中，该方法在面对外部力扰动和模型参数变化时均表现出显著的鲁棒性提升。
conclusion: 结合分式目标函数的极小极大对抗训练是提升强化学习策略在复杂环境下稳定性的有效途径。
---
