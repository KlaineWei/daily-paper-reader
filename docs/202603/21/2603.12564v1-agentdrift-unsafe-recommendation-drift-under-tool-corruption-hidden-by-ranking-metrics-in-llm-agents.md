...
motivation: 现有的排序指标仅衡量推荐内容而忽略了安全性，导致工具输出受损时智能体的安全风险被掩盖。
method: 提出一种配对轨迹协议，在清洁和受损工具输出条件下重放金融对话，并引入安全惩罚指标sNDCG。
result: 实验显示所有测试模型在工具受损时均表现出严重的安全性崩溃，且传统指标无法反映这种风险。
conclusion: 部署高风险领域的多轮智能体时，必须超越单轮质量评估，实施轨迹级的安全监控。
---
