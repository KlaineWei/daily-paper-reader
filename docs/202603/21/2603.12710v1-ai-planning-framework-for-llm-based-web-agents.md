...
tldr: 本研究针对基于大语言模型（LLM）的Web代理在任务规划中缺乏透明度的问题，提出了一种将Web任务视为序列决策过程的AI规划框架。该框架将现代代理架构映射到传统规划范式（如BFS、DFS），并引入了五种评估轨迹质量的新指标。通过对WebArena基准测试中794条人工标注轨迹的分析，验证了不同架构在成功率与技术准确性方面的权衡，为选择合适的代理架构提供了理论依据。
selection_source: fresh_fetch
motivation: 现有的LLM Web代理通常被视为黑盒，难以诊断其失败原因或理解其内部规划逻辑。
method: 将Web任务形式化为序列决策过程，并将代理架构映射到BFS、DFS等传统规划范式，同时提出五种新的轨迹评估指标。
result: "实验显示逐步执行代理在人类轨迹对齐上表现更好，而预先全量规划代理在元素准确率上高达89%。"
conclusion: 提出的规划框架和多维度评估指标对于根据具体应用约束选择和优化Web代理架构至关重要。
---
