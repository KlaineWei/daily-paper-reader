...
## 摘要
理解用户指令和周围环境中的物体空间关系，对于智能机器人系统协助人类完成各种任务至关重要。视觉语言模型（VLMs）的自然语言和空间推理能力具有增强机器人规划器在面对新任务、新物体和运动规范时泛化能力的潜力。虽然基础模型已被应用于任务规划，但目前尚不清楚它们在多大程度上具备执行用户运动偏好或约束（如与物体的期望距离、拓扑属性或运动风格偏好）所需的空间推理能力。在本文中，我们使用四种不同的查询方法，评估了四种最先进的 VLM 在机器人运动空间推理方面的能力。我们的结果表明，在使用性能最高的查询方法时，Qwen2.5-VL 在零样本（zero-shot）情况下达到了 71.4% 的准确率，微调后的较小模型达到了 75%，而 GPT-4o 的表现相对较低。我们评估了两种类型的运动偏好（物体接近度和路径风格），并分析了准确率与以 Token 数量衡量的计算成本之间的权衡。这项工作展示了将 VLM 集成到机器人运动规划流水线中的潜力。
