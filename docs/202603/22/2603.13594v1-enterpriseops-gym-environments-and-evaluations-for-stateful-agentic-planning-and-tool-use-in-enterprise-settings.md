...
## 摘要
大语言模型正从被动的信息提供者转变为旨在处理复杂工作流的主动智能体。然而，由于现有基准测试未能捕捉到专业环境的复杂性，特别是持续状态变化和严格访问协议下的长程规划需求，它们作为可靠 AI 员工在企业中的部署受到了阻碍。在这项工作中，我们推出了 EnterpriseOps-Gym，这是一个旨在评估现实企业场景下智能体规划能力的基准测试。具体而言，EnterpriseOps-Gym 包含一个容器化的沙箱，其中包含 164 张数据库表和 512 个功能工具，用以模拟真实世界的搜索摩擦。在该环境中，智能体在涵盖八个关键业务垂直领域（包括客户服务、人力资源和 IT）的 1,150 个专家策划任务上接受评估。我们对 14 个前沿模型的评估揭示了最先进模型的关键局限性：表现最好的 Claude Opus 4.5 仅实现了 37.4% 的成功率。进一步分析表明，提供理想的人类规划可将性能提高 14-35 个百分点，这表明战略推理是主要的瓶颈。此外，智能体经常无法拒绝不可行的任务（表现最好的模型仅达到 53.9%），从而导致意外且可能有害的副作用。我们的研究结果强调，目前的智能体尚未准备好进行自主的企业部署。更广泛地说，EnterpriseOps-Gym 为提升专业工作流中智能体规划的鲁棒性提供了一个具体的测试平台。
