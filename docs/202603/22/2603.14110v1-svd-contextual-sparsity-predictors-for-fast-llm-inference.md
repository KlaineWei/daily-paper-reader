...
tldr: "本研究针对大语言模型推理的高计算开销，提出了一种针对 ReGLU 结构前馈网络（FFN）的上下文稀疏性加速框架。该框架利用截断奇异值分解（SVD）构建免训练的稀疏模式预测器，并结合阈值校准算法和高效推理执行器。实验表明，在保持 90% 稀疏度的情况下，该方法实现了 1.8 倍的解码加速，且在复杂任务中精度损失极小，显著提升了 LLM 在边缘设备上的部署效率。"
selection_source: fresh_fetch
motivation: 现有的上下文稀疏性技术通常需要额外的模型训练来预测稀疏模式，增加了部署成本和复杂性。
method: 提出一种基于门控投影矩阵截断 SVD 的免训练预测方法，并配套阈值校准算法及支持 CUDA 和 CANN 的推理执行器。
result: "在 FFN 激活稀疏度平均为 90% 的模型上，实现了高达 1.8 倍的端到端解码加速，且数学和代码任务的精度下降低于 1%。"
conclusion: 该研究通过免训练的 SVD 预测技术，为在资源受限的边缘设备上实现大语言模型的高效部署提供了有效方案。
---
