...
tldr: 针对扩散模型在精细化生成（如文本渲染、组合性）方面的不足，本文提出了一种名为中心化奖励蒸馏（CRD）的强化学习微调框架。CRD通过在提示词内进行中心化处理，消除了难以计算的归一化常数，并引入了解耦采样器、KL锚定和自适应KL强度等技术，有效解决了扩散RL中的分布偏移和奖励作弊问题，在GenEval和OCR任务上达到了SOTA性能。
selection_source: fresh_fetch
motivation: 现有的扩散模型强化学习微调方法面临内存开销大、梯度方差高以及容易产生分布偏移和奖励作弊等不稳定性挑战。
method: 提出CRD框架，利用提示词内中心化消除归一化常数，并结合解耦采样、KL锚定及自适应KL强度来稳定训练过程。
result: 在GenEval和OCR奖励的文本生成图像实验中，CRD实现了快速收敛，并在优化奖励的同时显著减少了奖励作弊现象。
conclusion: CRD为扩散模型的强化学习微调提供了一种高效且稳健的方案，显著提升了模型对复杂指令的遵循能力和生成质量。
---
