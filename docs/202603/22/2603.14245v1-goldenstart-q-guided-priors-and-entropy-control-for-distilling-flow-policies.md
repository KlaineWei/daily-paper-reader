...
## 摘要
流匹配策略（Flow-matching policies）通过捕捉复杂的、多模态的动作分布，在强化学习（RL）中展现出巨大的潜力。然而，它们的实际应用往往受到极高的推理延迟和低效的在线探索的阻碍。尽管最近的研究采用了单步蒸馏（one-step distillation）来实现快速推理，但初始噪声分布的结构仍然是一个被忽视的因素，具有巨大的未开发潜力。这一被忽视的因素，以及控制策略随机性的挑战，构成了推进蒸馏流匹配策略的两个关键领域。为了克服这些局限性，我们提出了 GoldenStart (GSFlow)，这是一种具有 Q 引导先验和显式熵控制的策略蒸馏方法。我们引入了由条件变分自编码器（CVAE）建模的 Q 引导先验，而不是从无信息的噪声开始初始化生成过程。这种以状态为条件的先验将单步生成过程的起点重新定位到高 Q 值区域，有效地提供了一个“黄金起点”（golden start），使策略能够快速通向有前景的动作。此外，为了实现有效的在线探索，我们使蒸馏后的 Actor 能够输出随机分布而非确定性点。这由熵正则化控制，允许策略从纯粹的利用转向有原则的探索。我们的集成框架表明，通过设计生成起点并显式控制策略熵，可以实现高效且具有探索性的策略，从而架起了生成模型与实际 Actor-Critic 方法之间的桥梁。我们在离线和在线连续控制基准测试上进行了广泛的实验，结果表明我们的方法显著优于现有的最先进方法。代码将在 https://github.com/ZhHe11/GSFlow-RL 上发布。
