...
tags: ["query:slm-rl"]
score: 8.0
evidence: 将基于视觉的场景理解与强化学习及规划结合用于足式机器人
tldr: VIP-Loco是一个结合视觉感知与无限时界规划的足式机器人运动框架。针对MPC处理高维感知难和RL缺乏规划能力的痛点，该框架通过强化学习训练内部模型提取运动学特征，并在部署时将其集成到MPC中。实验证明，该方法在四足、双足及轮足机器人上均实现了在复杂地形（如楼梯、跳跃、攀爬）下的鲁棒且可解释的运动。
selection_source: carryover_cache
motivation: 旨在解决模型预测控制（MPC）难以处理高维视觉输入，以及强化学习（RL）在复杂地形中缺乏结构化规划能力的问题。
method: 提出一种集成视觉感知、强化学习与无限时界规划的框架，利用学习到的运动学特征增强MPC的规划能力。
result: 在四足、双足和轮足三种机器人形态上成功实现了斜坡、楼梯、跳跃和攀爬等多种高难度地形的稳定运动。
conclusion: VIP-Loco成功统一了感知与规划，为足式机器人在多样化动态环境中的鲁棒运动提供了一种可解释且高效的解决方案。
---
