...
tags: ["query:slm-rl"]
score: 8.0
evidence: 用于机器人操作和鲁棒性的视觉语言模型
tldr: "视觉语言模型（VLM）在机器人操作中表现出色但缺乏鲁棒性，尤其在面对易混淆物体时常因“捷径学习”导致识别错误。本文提出混淆感知上下文学习（CAICL）方法，通过定位和分析混淆源，并将其作为提示引导模型关注关键特征。在VIMA-Bench上的实验表明，该方法有效解决了捷径学习问题，成功率达85.5%，显著提升了模型在复杂场景下的稳定性和泛化能力。"
selection_source: carryover_cache
motivation: 针对视觉语言模型在机器人操作中因捷径学习而难以区分易混淆物体、导致系统鲁棒性不足的问题。
method: 提出CAICL方法，通过混淆定位与分析识别潜在干扰源，并利用提示词引导模型聚焦于关键区分特征。
result: "在VIMA-Bench基准测试中取得了85.5%的成功率，并在不同泛化程度的任务中表现出良好的稳定性。"
conclusion: CAICL通过引入混淆感知机制，有效克服了VLM的捷径学习缺陷，增强了机器人在复杂环境下的操作精度。
---

## 摘要
视觉语言模型 (VLMs) 显著提高了机器人操纵的泛化能力。然而，基于 VLM 的系统通常缺乏鲁棒性，导致不可预测的错误，特别是在涉及易混淆物体的场景中。我们的初步分析表明，这些失败主要是由 VLM 固有的捷径学习 (shortcut learning) 问题引起的，限制了它们准确区分易混淆特征的能力。为此，我们提出了混淆感知上下文学习 (CAICL)，这是一种增强 VLM 在机器人操纵易混淆场景中性能的方法。该方法首先进行混淆定位和分析，识别潜在的混淆源。然后，这些信息被用作 VLM 的提示，使其专注于最可能导致误识别的特征。在 VIMA-Bench 上的广泛实验表明，CAICL 有效地解决了捷径学习问题，实现了 85.5% 的成功率，并在具有不同泛化程度的任务中表现出良好的稳定性。
