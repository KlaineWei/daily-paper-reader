...
tags: ["query:slm-rl"]
score: 8.0
evidence: 具身智能的轻量级多任务模仿学习
tldr: 针对多任务双臂操作中存在的任务干扰和负迁移问题，本文提出了MoE-ACT框架。该框架通过在ACT的Transformer编码器中集成稀疏专家混合（MoE）模块，实现了多任务动作分布的解耦。结合FiLM动态调制和多尺度交叉注意力机制，MoE-ACT将视觉与语言指令深度融合，显著提升了机器人在复杂环境下的多任务执行能力和泛化性能。
selection_source: carryover_cache
motivation: 旨在解决通用机器人策略在多任务训练中因任务间分布差异导致的干扰和负迁移挑战。
method: 在ACT架构中引入稀疏MoE模块解耦任务策略，并结合FiLM调制与多尺度交叉注意力增强语言条件下的动作生成。
result: "实验表明MoE-ACT在仿真和真实双臂场景中表现优异，平均成功率比原始ACT提升了33%。"
conclusion: MoE-ACT通过专家混合机制有效提升了多任务双臂操作的鲁棒性与泛化能力，为具身智能应用提供了有力支持。
---

## 摘要
机器人通过统一策略处理多任务的能力，对于在现实家庭和工业应用中部署具身智能至关重要。然而，在训练通用机器人策略时，任务间的分布外差异往往会导致严重的任务干扰和负迁移。为应对这一挑战，我们提出了一种用于双臂操作的轻量级多任务模仿学习框架，称为混合专家增强型动作分块 Transformer（MoE-ACT），该框架将稀疏混合专家（MoE）模块集成到 ACT 的 Transformer 编码器中。MoE 层将统一的任务策略分解为独立调用的专家组件。通过自适应激活，它在潜空间中自然地解耦了多任务动作分布。在解码过程中，特征线性调制（FiLM）动态调制动作标记，以提高动作生成与任务指令之间的一致性。同时，多尺度交叉注意力机制使策略能够同时关注低层和高层语义特征，为机器人操作提供丰富的视觉信息。我们进一步融入了文本信息，将框架从纯视觉模型转变为以视觉为中心、语言条件的动作生成系统。在仿真和真实双臂设置中的实验验证表明，MoE-ACT 显著提升了多任务性能。具体而言，MoE-ACT 的成功率比原始 ACT 平均高出 33%。这些结果表明，MoE-ACT 在复杂的双臂多任务操作环境中具有更强的鲁棒性和泛化能力。我们的开源项目页面见 https://j3k7.github.io/MoE-ACT/。
