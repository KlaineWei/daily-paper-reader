...
tags: ["query:slm-rl"]
score: 7.0
evidence: 多模态模型中的测试时强化学习
tldr: 针对统一多模态模型（UMM）在文生图任务中推理阶段无法积累知识的问题，本文提出 Meta-TTRL 框架。该框架利用模型内在的元认知监控信号指导测试时参数优化，实现了从实例级到能力级的自我进化。实验证明，Meta-TTRL 在 Janus-Pro 等多个模型上显著提升了组合推理能力和生成质量，揭示了元认知协同在测试时强化学习中的关键作用。
selection_source: fresh_fetch
motivation: 现有的测试时缩放方法仅能实现单次推理的改进，缺乏从先前推理中学习并积累知识以提升模型整体能力的能力。
method: 提出一种元认知测试时强化学习框架，利用模型内在的元认知监控信号引导参数优化，实现测试阶段的自我改进。
result: 在 Janus-Pro、BAGEL 和 Qwen-Image 等多个模型上的实验表明，该方法显著增强了组合推理能力并在多个文生图基准上取得领先。
conclusion: Meta-TTRL 验证了测试时强化学习在文生图领域的有效性，并指出元认知信号与优化机制的协同是实现自我改进的关键。
---
