...
tags: ["query:slm-rl"]
score: 6.0
evidence: 用于降低大模型成本和延迟的轻量级代理模型
tldr: 针对数据库中集成大语言模型（LLM）进行AI查询时面临的高成本和高延迟挑战，本文评估了一种基于轻量级代理模型的近似查询方法。通过在嵌入向量上利用廉价且准确的代理模型，该方法在保持甚至提升准确率的同时，实现了语义过滤（AI.IF）和语义排序（AI.RANK）操作超过100倍的成本与延迟降低。研究展示了在BigQuery和AlloyDB中的架构实现，为大规模结构化与非结构化数据融合查询提供了高效方案。
selection_source: fresh_fetch
motivation: 直接在数据库中调用大语言模型进行大规模AI查询会导致极高的计算成本和无法接受的响应延迟。
method: 利用嵌入向量训练轻量级代理模型来近似LLM的推理结果，并针对不同数据库场景设计了在线和离线训练架构。
result: 在多个基准测试中实现了超过100倍的成本和延迟缩减，同时在千万级数据规模下保持或提升了查询准确率。
conclusion: 轻量级代理模型是实现高性能、低成本数据库AI查询的有效手段，能够显著提升大规模语义分析的实用性。
---

## 摘要
几家数据仓库和数据库提供商最近引入了名为“AI查询”（AI Queries）的SQL扩展，允许用户在SQL中指定由大语言模型（LLM）评估的函数和条件，从而显著扩展了在结构化和非结构化数据组合上可表达的查询类型。LLM 提供了卓越的语义推理能力，使其成为处理融合结构化与非结构化数据的复杂且细致查询的重要工具。尽管功能极其强大，但当这些 AI 查询被调用数千次时，其成本可能会变得高得令人望而却步。本文对近期的一种 AI 查询近似方法进行了广泛评估，该方法使低成本分析和数据库应用能够从 AI 查询中受益。该方法为语义过滤（AI.IF）算子带来了超过 100 倍的成本和延迟降低，并为语义排序（AI.RANK）带来了显著提升。成本和性能的提升源于在嵌入向量（embedding vectors）上利用廉价且准确的代理模型。我们展示了尽管在延迟和成本方面取得了巨大进步，这些代理模型在各种基准数据集（包括拥有 1000 万行的扩展 Amazon 评论基准）上仍能保持准确性，甚至偶尔能提高准确性。我们在 Google BigQuery 中为该方法提出了一种适用于纯在线（即时）查询的 OLAP 友好型架构，并在 AlloyDB 中提出了一种低延迟的 HTAP 数据库友好型架构，后者可以通过将代理模型训练移至线下进一步降低延迟。我们还提出了加速代理模型训练的技术。
