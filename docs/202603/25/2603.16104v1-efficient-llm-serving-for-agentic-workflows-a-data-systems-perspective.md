...
tags: ["query:llmrec"]
score: 6.0
evidence: 智能体工作流和LLM调用的服务框架
tldr: 本研究针对智能体工作流中存在的冗余和依赖问题，从数据系统视角重新审视LLM推理。提出了Helium框架，将工作流建模为查询计划，并将LLM调用视为一级算子。通过主动缓存和缓存感知调度技术，Helium实现了跨提示词和KV状态的高效复用，显著提升了端到端推理效率，为构建高性能智能体系统提供了新思路。
selection_source: fresh_fetch
motivation: 现有的LLM推理系统主要优化单次调用，忽视了智能体工作流中跨调用依赖导致的严重冗余和低效。
method: 提出Helium框架，将智能体负载建模为查询计划，并集成主动缓存与缓存感知调度以最大化资源复用。
result: 在多种工作流负载下，Helium相比最先进的智能体推理系统实现了高达1.56倍的加速。
conclusion: 跨工作流的端到端优化对于构建可扩展且高效的LLM智能体系统至关重要。
---
