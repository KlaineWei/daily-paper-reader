...
method: 通过训练轻量级代理模型根据评分准则预测偏好，并将预测准确率作为强化学习的奖励信号来引导模型生成高质量准则。
result: 在VL-Reward Bench等多个榜单上取得SOTA性能，且仅需约5万条数据，效率是同类方法的四倍。
conclusion: 显式优化评分准则的质量能显著增强奖励模型的内部一致性与可迁移性，是提升VLM评估性能的有效途径。
---
