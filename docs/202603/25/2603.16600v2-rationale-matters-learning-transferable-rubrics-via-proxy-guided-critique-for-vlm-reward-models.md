...
tags: ["query:slm-rl"]
score: 6.0
evidence: 强化学习用于VLM奖励模型和准则生成
tldr: 本文针对视觉语言模型（VLM）生成式奖励模型中评分准则（rubric）缺乏直接优化的问题，提出了 Proxy-GRM 框架。该方法通过引入轻量级代理模型来验证准则质量，将代理模型仅依据准则预测偏好的准确率作为强化学习的奖励信号，从而引导模型生成一致性强且可迁移的评分准则。实验证明，Proxy-GRM 在多个奖励模型基准测试中仅需少量数据即可达到 SOTA 性能，并显著提升了测试时的奖励准确度。
selection_source: fresh_fetch
motivation: 现有的生成式奖励模型通常将评分准则视为附带产物，缺乏直接的优化手段，且依赖昂贵的 LLM 评估导致训练引导受限。
method: 引入轻量级代理模型作为验证器，通过强化学习将代理模型对准则的预测准确性转化为优化奖励信号，以增强准则的质量。
result: 在多个多模态奖励模型榜单上超越了使用四倍数据训练的现有方法，并展现出极强的准则可迁移性和数据效率。
conclusion: 显式优化评分准则的质量是提升 VLM 奖励模型性能的关键，Proxy-GRM 为构建高效、可解释的奖励模型提供了新方案。
---
