...
tags: ["query:slm-rl"]
score: 8.0
evidence: 基于模型预测控制的多智能体任务与运动规划
tldr: 针对多智能体任务与运动规划（TAMP）中约束序列处理困难、无法应对动态任务分配和环境扰动的问题，本文提出了GoC-MPC框架。该方法将约束图与模型预测控制相结合，通过跟踪3D关键点定义约束，支持部分有序任务和动态智能体协作。GoC-MPC无需训练数据或环境模型，仅凭视觉观测即可实现高效、鲁棒的多智能体操纵，在复杂环境下显著提升了计算速度和任务成功率。
selection_source: fresh_fetch
motivation: 现有的多智能体TAMP方法在处理部分有序任务和动态智能体分配时缺乏灵活性，难以应对实时扰动。
method: 提出GoC-MPC框架，通过在3D关键点上定义约束图并集成到MPC中，实现对复杂约束序列的在线反应式规划。
result: 实验表明，该方法在多智能体操纵任务中比基准方法具有更高的成功率、更快的计算速度和更短的路径长度。
conclusion: GoC-MPC为现实世界扰动下的多智能体协作操纵提供了一种高效、鲁棒且无需预训练的通用解决方案。
---
