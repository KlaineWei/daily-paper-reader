...
method: 提出SCRL框架，结合了严格筛选可靠共识的选择性正向伪标签和基于生成不确定性剔除错误解的熵门控负向伪标签。
result: 在多个推理基准测试中，SCRL相比现有基准取得了显著的性能提升，并在受限的采样预算下表现出更强的训练稳定性。
conclusion: SCRL通过引入负向监督和选择性过滤机制，有效解决了测试时强化学习中的伪标签噪声问题，增强了大语言模型的推理可靠性。
---
