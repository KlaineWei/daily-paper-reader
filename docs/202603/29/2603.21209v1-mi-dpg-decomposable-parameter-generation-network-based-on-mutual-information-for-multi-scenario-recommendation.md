...
## 摘要
转化率（CVR）预测模型在推荐和广告系统中起着至关重要的作用。最近关于多场景推荐的研究表明，学习一个统一的模型来服务于多个场景对于提高整体性能是有效的。然而，如何在低模型参数成本下提高跨场景的模型预测性能仍然具有挑战性，且目前的解决方案难以鲁棒地建模多场景的多样性。在本文中，我们为多场景 CVR 预测提出了 MI-DPG，它以更高效且有效的方式为每个场景学习场景条件的动态模型参数。具体而言，我们引入了一个辅助网络来生成场景条件的动态权重矩阵，这些矩阵是通过将分解的场景特定和场景共享的低秩矩阵相结合而获得的，具有较高的参数效率。对于每个场景，通过权重矩阵对骨干模型参数进行加权，有助于使模型参数针对不同场景进行专门化。这不仅可以调节骨干模型的完整参数空间，还能提高模型的有效性。此外，我们设计了一种互信息正则化，通过最大化场景感知输入与场景条件动态权重矩阵之间的互信息，来增强不同场景间模型参数的多样性。在三个真实世界数据集上的实验表明，MI-DPG 显著优于以往的多场景推荐模型。
