...
tldr: 针对大视觉语言模型（LVLM）在强化学习中视觉信息利用不足的问题，本文提出KAWHI插件式奖励重加权机制。该方法通过分层几何聚合定位关键视觉区域，并利用结构化归因识别视觉关键注意力头，实现段落级信用分配，将空间视觉证据与推理步骤对齐。实验证明KAWHI能显著提升GRPO等主流强化学习算法在多模态推理任务上的表现。
source: arxiv
...
motivation: 现有LVLM强化学习方法缺乏对视觉信息的显式建模，导致视觉表示与优化过程脱节，限制了多模态推理能力的提升。
method: 提出KAWHI机制，通过分层几何聚合定位显著区域并结合结构化归因进行段落级奖励重加权，实现视觉证据与推理步骤的精准对齐。
result: 在多个推理基准测试上的实验表明，KAWHI作为通用增强模块，能持续提升GRPO和GSPO等多种统一奖励优化方法的性能。
conclusion: KAWHI通过强化视觉表示与强化学习的耦合，有效解决了LVLM在可验证奖励学习中的结构性表征瓶颈。
---

## 摘要
基于可验证奖励的强化学习 (RLVR) 显著增强了大语言模型在抽象推理任务中的推理能力。然而，其在大视觉语言模型 (LVLMs) 中的应用仍受限于结构性的表示瓶颈。现有方法通常缺乏对视觉信息的显式建模和有效利用，导致视觉表示无法与强化学习优化过程紧密耦合，从而限制了多模态推理性能的进一步提升。为了解决这一局限性，我们提出了 KAWHI（Key-Region Aligned Weighted Harmonic Incentive，关键区域对齐加权调和激励），这是一种即
