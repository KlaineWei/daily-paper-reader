...
## 摘要
在多轮会话中与用户交互的 AI 智能体需要持久的长期记忆，以维持连贯且个性化的行为。目前的方法要么依赖于扁平的检索增强生成（RAG），这会丢失记忆间的结构关系；要么使用记忆压缩和向量检索，无法捕捉多轮对话的关联结构。文献中虽有少量基于图的技术被提出，但它们仍受限于枢纽节点主导的检索，且在演化记忆上的层级推理能力较弱。我们提出了 GAAMA，一种图增强关联记忆系统。该系统通过三步流水线构建以概念为中介的层级知识图谱：(1) 从原始对话中逐字保留片段（episode）；(2) 基于大语言模型（LLM）提取原子事实和主题级概念节点；(3) 合成高阶反思（reflection）。生成的图包含四种节点类型（片段、事实、反思、概念），由五种结构化边类型连接，其中概念节点提供了跨越式的遍历路径，补充了语义相似度。检索过程通过加法评分函数，将基于余弦相似度的 k-最近邻搜索与感知边类型的个性化 PageRank（PPR）相结合。在 LoCoMo-10 基准测试（涵盖 10 个多轮对话中的 1,540 个问题）上，GAAMA 达到了 78.9% 的平均奖励，优于经过调优的 RAG 基准（75.0%）、HippoRAG（69.9%）、A-Mem（47.2%）和 Nemori（52.1%）。消融实验表明，将基于图遍历的排序（个性化 PageRank）与语义搜索相结合，相较于仅在图节点上进行语义搜索，性能得到了持续提升（整体提升 1.0 个百分点）。
