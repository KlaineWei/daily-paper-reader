...
tldr: 本研究提出了GraSP-STL框架，旨在解决离线数据集下的零样本信号时序逻辑（STL）规划问题。该方法无需环境交互或任务重训，通过从离线数据中学习目标条件价值函数并构建有向图抽象，将复杂的STL规划转化为图搜索问题。该框架实现了可重用的可达性学习与任务规划的分离，支持长时程规划并能泛化至未见的STL任务。
source: arxiv
