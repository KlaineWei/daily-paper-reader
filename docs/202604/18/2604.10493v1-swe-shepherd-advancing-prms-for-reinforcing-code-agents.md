...
tldr: 针对LLM代码智能体在处理复杂软件工程任务时缺乏细粒度反馈、探索效率低的问题，本文提出SWE-Shepherd框架。该框架引入过程奖励模型（PRM），通过在SWE-Bench轨迹上构建动作级奖励数据集并训练轻量级模型，为智能体的中间决策提供密集监督。实验证明，该方法显著提升了交互效率和动作质量，为强化代码智能体提供了新思路。
source: arxiv
...
motivation: 现有的代码智能体依赖静态提示或启发式方法，缺乏对中间决策的细粒度反馈，导致长程推理效率低下且易出错。
method: 提出SWE-Shepherd框架，利用SWE-Bench轨迹构建动作级奖励数据集，并训练轻量级PRM在推理阶段评估并引导智能体的候选动作。
result: 在SWE-Bench Verified上的实验表明，该方法显著提高了智能体的交互效率和动作质量，能更有效地引导高奖励决策。
conclusion: 引入过程奖励模型能有效增强代码智能体的决策能力，但将中间奖励与最终任务成功率完全对齐仍是未来面临的挑战。
---
