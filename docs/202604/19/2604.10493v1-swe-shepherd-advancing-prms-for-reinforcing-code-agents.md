...
tags: ["query:slm-rl"]
score: 7.0
evidence: 过程奖励模型用于增强智能体的长程推理
tldr: 针对LLM智能体在处理复杂软件工程任务时缺乏中间反馈、探索效率低的问题，本文提出了SWE-Shepherd框架。该框架引入了过程奖励模型（PRM），通过在SWE-Bench轨迹上构建动作级奖励数据集并训练轻量化模型，为代码智能体的每一步决策提供密集监督。实验表明，该方法显著提升了智能体在仓库级代码编辑和导航中的交互效率与动作质量。
source: arxiv
...
motivation: 现有的代码智能体主要依赖静态提示或启发式规则，缺乏对中间决策的细粒度反馈，导致长程推理中容易出现错误累积和低效探索。
method: 提出SWE-Shepherd框架，利用SWE-Bench轨迹构建动作级奖励数据集，训练轻量级PRM在推理阶段评估并引导智能体选择高奖励的候选动作。
result: 在SWE-Bench Verified基准测试中，该方法有效提高了智能体的交互效率和动作准确性。
conclusion: 引入过程奖励模型能显著增强代码智能体的决策能力，但也揭示了中间奖励与最终任务成功之间在对齐上的挑战。
---

## 摘要
由于需要在大型且不断演进的代码库上进行长程推理，并在相互依赖的操作之间做出一致的决策，自动化处理现实世界的软件工程任务对于基于大语言模型（LLM）的智能体而言仍然具有挑战性。现有方法通常依赖静态提示策略或手工设计的启发式规则来选择代码编辑、文件导航和测试执行等操作，但缺乏对中间决策的细粒度反馈。这导致了低效的探索、错误传播以及脆弱的解决方案轨迹。为了解决这一局限性，我们提出了 SWE-Shepherd，这是一个引入过程奖励模型（PRMs）的框架，旨在为仓库级代码智能体提供密集的步骤级监督。利用来自 SWE-Bench 的轨迹，我们构建了一个操作级奖励数据集，并在基础 LLM 上训练了一个轻量级奖励模型，用以评估中间操作的效用。在推理过程中，PRM 评估候选操作，并在无需完整强化学习的情况下引导智能体做出更高奖励的决策。在 SWE-Bench Verified 上的实验证明了交互效率和操作质量的提升，同时也揭示了将中间奖励与最终任务成功对齐所面临的挑战。
