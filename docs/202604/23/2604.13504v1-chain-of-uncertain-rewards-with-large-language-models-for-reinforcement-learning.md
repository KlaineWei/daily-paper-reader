...
tags: ["query:slm-rl"]
score: 7.0
evidence: LLM用于强化学习中的奖励函数设计
tldr: 针对强化学习中奖励函数设计繁琐且效率低下的问题，本文提出CoUR框架。该框架利用大语言模型，通过代码不确定性量化和相似性选择机制，结合文本与语义分析来识别并重用高效的奖励组件。通过对解耦后的奖励项进行贝叶斯优化，CoUR实现了更高效、稳健的奖励函数搜索，在多个复杂机器人仿真任务中显著提升了学习性能并降低了计算评估成本。
source: arxiv
...
motivation: 传统奖励函数设计依赖大量人工且存在评估冗余，且现有自动化方法往往忽略了中间决策点的局部不确定性。
method: 提出CoUR框架，利用LLM生成奖励代码，并结合不确定性量化、相似性筛选机制以及针对解耦奖励项的贝叶斯优化进行迭代改进。
result: 在IsaacGym和双手操作基准测试的29个任务中，CoUR不仅实现了更优的性能，还显著降低了奖励函数的评估开销。
conclusion: CoUR通过集成大模型与不确定性建模，为高效、自动化的强化学习奖励函数设计提供了一种稳健且低成本的新方案。
---

## 摘要
设计有效的奖励函数是强化学习（RL）的基石，但由于传统方法固有的低效和不一致性，这仍然是一个具有挑战性且劳动密集的过程。现有方法通常依赖大量的辅助设计和评估步骤，这些步骤容易产生冗余，并忽略了中间决策点的局部不确定性。为了应对这些挑战，我们提出了不确定奖励链（CoUR），这是一个集成大语言模型（LLM）的新型框架，旨在简化强化学习环境中的奖励函数设计与评估。具体而言，CoUR 引入了代码不确定性量化以及一种相似性选择机制，该机制结合了文本和语义分析，以识别并重用最相关的奖励函数组件。通过减少冗余评估并对解耦的奖励项利用贝叶斯优化，CoUR 能够更高效、更稳健地搜索最优奖励反馈。我们在 IsaacGym 的九个原始环境以及 Bidexterous Manipulation 基准测试的所有 20 个任务中对 CoUR 进行了全面评估。实验结果表明，CoUR 不仅实现了更好的性能，还显著降低了奖励评估的成本。
